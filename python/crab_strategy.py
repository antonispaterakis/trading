...
def generate_signals(data: Dict, p: Dict) -> Dict[str, List]:
    close, high, low, volume = data["close"], data["high"], data["low"], data.get("volume", [])
    n = len(close)
    
    # Calculate indicators
    a = atr(high, low, close, p.get("atr_len", 14))
    adx_line = adx(high, low, close, p.get("adx_len", 14))
    bb_basis, bb_upper, bb_lower = bollinger_bands(close, p.get("bb_len", 20), p.get("bb_mult", 2.0))
    macd_line, sig_line, hist = macd(close, p.get("macd_fast", 12), p.get("macd_slow", 26), p.get("macd_sig", 9))
    ...
    for i in range(1, n):
        if (a[i] is None or adx_line[i] is None or bb_upper[i] is None or 
            hist[i] is None or hist[i-1] is None or vma[i] is None):
            continue
