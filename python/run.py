...
Examples
--------
  python3 run.py                                  # walk-forward on BTCUSDT 1h (baseline)
  python3 run.py --strategy crab                  # walk-forward on BTCUSDT 1h (crab scalping)
...
def fmt(m):
    return (f'trades={m["trades"]:>3}  win={m["win_rate"]:>4.0f}%  PF={_pf(m):>5}  '
...
def run_one(symbol, interval, bars, risk, mode, strategy_name):
    # Dynamically import the chosen strategy
    ...
    segs = walk_forward(data, p, generate_signals_fn=generate_signals, param_grid=PARAM_GRID, bars_per_year=bpy)
    if not segs:
    ...
    for i, s in enumerate(segs):
        ...
        bp = s["params"]
        # Print relevant params depending on strategy
        if strategy_name == "baseline":
    ...
    agg = {
        ...
        "avg_win": statistics.mean(s["metrics"]["win_rate"] for s in segs) if segs else 0.0,
        "avg_dd": statistics.mean(s["metrics"]["max_drawdown"] for s in segs) if segs else 0.0,
        "avg_sharpe": statistics.mean(s["metrics"]["sharpe"] for s in segs) if segs else 0.0,
    }
    print("  " + "-" * 60)
    print(f'  AGGREGATE OOS: return={agg["compounded_return"]:+.1f}%  '
...
def main():
    ap = argparse.ArgumentParser(description="Honest, walk-forward-validated trading backtester.")
    ...
    ap.add_argument("--mode", choices=["backtest", "walkforward"], default="walkforward")
    ap.add_argument("--strategy", choices=["baseline", "crab"], default="crab")
    ap.add_argument("--portfolio", default="", help="comma-separated symbols for a diversified run")
    args = ap.parse_args()
    ...
    if args.portfolio:
        ...
        aggs = []
        for sym in symbols:
            a = run_one(sym, args.interval, args.bars, args.risk, "walkforward", args.strategy)
            if a:
                aggs.append(a["compounded_return"])
        if aggs:
            print("\n=== PORTFOLIO (equal-weight average of per-market OOS returns) ===")
            print(f"  mean OOS return across {len(aggs)} markets: {statistics.mean(aggs):+.1f}%")
            print("  Diversification across uncorrelated markets is the one real free lunch —")
            print("  a blended result that beats the worst single market is the goal, not a hero trade.")
    else:
        run_one(args.symbol, args.interval, args.bars, args.risk, args.mode, args.strategy)
